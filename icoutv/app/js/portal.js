...
async function carregarDadosCliente(user) {
    ...
    try {
        ...
        if (msgSnap.exists()) {
            ...
            // Filtrar apenas mensagens ativas e para o status do cliente, ordenar por ordem
            const mensagensParaCliente = lista
                .filter(m => m.destino === statusDaConta && m.ativa !== false)
            ...
            if (mensagensParaCliente.length > 0) {
                ...
                const contadorEl = document.getElementById('modal-mensagem-contador');
                const btnProxima = document.getElementById('btn-proxima-modal');
                ...
                if (modalEl && conteudoEl) {
                    ...
                    function exibirMensagemModal(index) {
                        ...
                        conteudoEl.innerHTML = `<div class="modal-mensagem-item ${tiposClasse[statusDaConta] || 'msg-info'}">${iconeSvg}<span>${escapeHTML(m.texto)}</span></div>`;
                        if (contadorEl) {
                            contadorEl.textContent = totalMsgs > 1 ? `${index + 1} de ${totalMsgs}` : '';
                        }
                        const isUltima = index >= totalMsgs - 1;
                        // Só permitir fechar na última mensagem
                        if (btnFecharX) btnFecharX.style.display = isUltima ? '' : 'none';
                        if (totalMsgs > 1 && !isUltima) {
                            if (btnProxima) btnProxima.style.display = 'block';
                            if (btnEntendido) btnEntendido.style.display = 'none';
                        } else {
                            if (btnProxima) btnProxima.style.display = 'none';
                            if (btnEntendido) btnEntendido.style.display = 'block';
                        }
                    }
                    ...
                    // Botão "Próxima" avança para a próxima mensagem
                    if (btnProxima) {
