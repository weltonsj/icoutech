...
async function carregarDadosCliente(user) {
    ...
    try {
        ...
        if (msgSnap.exists()) {
            ...
            const bannerEl = document.getElementById('mensagens-admin-banner');
            if (mensagensParaCliente.length > 0 && bannerEl) {
                const tiposBanner = { 'ativa': 'banner-info', 'inativa': 'banner-aviso', 'excluida': 'banner-erro' };
                ...
                bannerEl.innerHTML = mensagensParaCliente.map(m =>
                    `<div class="mensagem-portal-banner ${tiposBanner[statusDaConta] || 'banner-info'}">${iconeSvg}<span>${m.texto}</span></div>`
                ).join('');
...
// ---------- Visualizar PDF (abre em nova aba sem perder estado) ----------
